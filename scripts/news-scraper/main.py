...
3. ТЕКСТ НОВОСТИ (content):
   - 300-600 слов, пиши от третьего лица (НЕ от первого, НЕ "я", НЕ "мы")
   - ОБЯЗАТЕЛЬНО дели текст на 4-6 смысловых абзацев с подзаголовками
   - Каждый подзаголовок на отдельной строке, выделен ** с двух сторон: **Что произошло**
   - После подзаголовка — двойной перенос строки, затем текст абзаца
   - Между абзацами — двойной перенос строки
   - Структура абзацев: Что произошло → Кого касается → Что изменится → Что делать бизнесу
   - Конкретные факты, цифры, даты, ссылки на статьи НК РФ
   - Без воды, общих фраз и рассуждений от первого лица
